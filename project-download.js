...
window.downloadFilesArchive = (title, files) => { const download = document.createElement('a'); const url = URL.createObjectURL(zipStore(files)); download.href = url; download.download = `${(title || '下载内容').replace(/[\\/:*?"<>|]/g, '_')}.zip`; download.click(); setTimeout(() => URL.revokeObjectURL(url), 1000); };
window.downloadProjectArchive = async (title, tasks) => {
  ...
  window.downloadFilesArchive(title || '项目', files);
};
