...
function renderRecentTasks(){const tasks=getRecentTasks();recent.list.innerHTML=tasks.length?tasks.map(task=>`<article class="recent-item"><div class="recent-icon">✦</div><div><strong>${task.title||'未命名海报'}</strong><small>${task.mode} · ${task.time} · ${task.status||'已完成'}</small></div>${task.status==='生成中'?'<span class="recent-running">生成中</span>':task.session===recentSession?`<a href="${task.url}" download="${task.fileName}">下载</a>`:'<span class="recent-stale">请重新生成</span>'}</article>`).join(''):'<div class="recent-empty">还没有生成记录。完成第一张海报后，会显示在这里。</div>';}
...
recent.button.addEventListener('click', event => { event.preventDefault(); renderRecentTasks(); recent.dialog.showModal(); });
recent.close.addEventListener('click', () => recent.dialog.close());
ui.generate.addEventListener('click',async()=>{
  ...
  const taskId=`task-${Date.now()}`;saveRecentTask({id:taskId,title:ui.title.value.trim(),mode:modeLabels[ui.mode],time:new Date().toLocaleString('zh-CN',{hour:'2-digit',minute:'2-digit'}),status:'生成中',session:recentSession});
  if(importedItem){importedItem.generationStatus='生成中';window.refreshImportedGenerationUI?.();}else{ui.generate.classList.add('loading');ui.generate.querySelector('span').textContent='AI 正在生成…';ui.note.lastElementChild.textContent='云雾正在重构画面并预留文字区域，请稍候。';}
